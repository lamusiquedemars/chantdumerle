...
class PaymentIntent extends Stancer\Core\AbstractObject
{
    ...
    public final const ENDPOINT = 'payment_intents';
    public final const API_VERSION = Stancer\Enum\ApiVersion::VERSION_2;
    /**
     * @phpstan-var array<string, DataModel>
     */
    protected array $dataModel = ['amount' => ['desc' => 'Intent amount', 'required' => \true, 'type' => self::INTEGER], 'billingAddress' => ['desc' => 'Billing address', 'onlyId' => \true, 'type' => Stancer\Address::class], 'capture' => ['desc' => 'Capture immediately the payment', 'type' => self::BOOLEAN], 'card' => ['changed' => [['sinceVersion' => Stancer\Enum\ApiVersion::VERSION_2, 'onlyID' => \true]], 'desc' => 'Card object', 'type' => Stancer\Card::class], 'currency' => ['desc' => 'Processed currency', 'required' => \true, 'type' => Stancer\Currency::class], 'customer' => ['changed' => [['sinceVersion' => Stancer\Enum\ApiVersion::VERSION_2, 'onlyID' => \true]], 'desc' => 'Customer object', 'type' => Stancer\Customer::class], 'description' => ['desc' => 'Intent description', 'size' => ['min' => 3, 'max' => 64], 'type' => self::STRING], 'metadata' => ['desc' => 'Arbitrary metadata', 'type' => self::MIXED], 'methodsAllowed' => ['desc' => 'List of payment methods allowed for this intent', 'list' => \true, 'type' => Stancer\Payment\MethodsAllowed::class], 'orderId' => ['desc' => 'Order identifier', 'size' => ['min' => 1, 'max' => 36], 'type' => self::STRING], 'payment' => ['desc' => 'Finalized payment', 'restricted' => \true, 'type' => Stancer\Payment::class], 'returnUrl' => ['desc' => 'URL to redirect back your customer after processing the payment', 'size' => ['min' => 1, 'max' => 65536], 'type' => self::STRING], 'sepa' => ['changed' => [['sinceVersion' => Stancer\Enum\ApiVersion::VERSION_2, 'onlyID' => \true]], 'desc' => 'SEPA object', 'type' => Stancer\Sepa::class], 'shippingAddress' => ['desc' => 'Shipping Address', 'onlyId' => \true, 'type' => Stancer\Address::class], 'status' => ['desc' => 'Status of the intent', 'restricted' => \true, 'type' => Stancer\PaymentIntent\Status::class], 'threeds' => ['desc' => 'Ask for an authenticated payment', 'type' => Stancer\ThreeDomainsSecure\Status::class], 'url' => ['desc' => 'Payment page URL', 'restricted' => \true, 'type' => self::STRING]];
    /**
     ...
     */
    #[\Stancer\Scoped\Isolated\Override]
    ...
    public static function filterListParams(array $terms) : array
    {
        ...
        if (\array_key_exists('card', $terms)) {
            ...
            if (\strlen($params['card']) !== 29) {
                throw new Stancer\Exceptions\InvalidSearchCardFilterException('A valid Card reference must have 29 characters.');
            }
        }
        if (\array_key_exists('order_id', $terms)) {
            ...
            if (\strlen($terms['order_id']) > 36 || !$terms['order_id']) {
                throw new Stancer\Exceptions\InvalidSearchOrderIdFilterException('A valid order ID must be between 1 and 36 characters.');
            }
        }
        if (\array_key_exists('sepa', $terms)) {
            ...
            if (\strlen($params['sepa']) !== 29) {
                throw new Stancer\Exceptions\InvalidSearchSepaFilterException('A valid SEPA reference must have 29 characters.');
            }
        }
        return $params;
    }
    ...
    public function setMethodsAllowed(array $methods) : static
    {
        ...
        foreach ($methods as $method) {
            try {
                ...
                $params = [$method, \implode(', ', \array_map($cast, Stancer\Payment\MethodsAllowed::cases()))];
        ...
        $status = [null, Stancer\PaymentIntent\Status::REQUIRE_PAYMENT_METHOD, Stancer\PaymentIntent\Status::UNPAID];
        if (\in_array($this->status, $status, \true) && \in_array(Stancer\Payment\MethodsAllowed::SEPA, $new, \true) && $currency && $currency !== Stancer\Currency::EUR) {
