    ...
    public function request(string $method, string $uri, array $options = []) : Psr\Http\Message\ResponseInterface
    {
        ...
        if ($error || $code >= 400) {
            ...
            if ($body->getSize()) {
                ...
                if (\json_last_error() === \JSON_ERROR_NONE && \is_array($json) && \array_key_exists('error', $json)) {
                    if (\is_array($json['error']) && \array_key_exists('message', $json['error']) && $json['error']['message']) {
