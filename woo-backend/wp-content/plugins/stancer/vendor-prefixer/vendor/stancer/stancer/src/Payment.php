 ...
 * @internal
 */
#[Stancer\Core\Documentation\AddMethod('addRefunds', ['Stancer\\Refund $refund'], '$this', stan: \true)]
...
#[Stancer\Core\Documentation\AddProperty('refunds', restricted: \true)]
class Payment extends Stancer\Core\AbstractObject
{
    ...
    public final const ENDPOINT = 'payments';
    /**
     * @phpstan-var array<string, DataModel>
     */
    protected array $dataModel = ['amount' => ['desc' => 'Transaction amount', 'required' => \true, 'type' => self::INTEGER], 'auth' => ['desc' => 'Auth object, must be set for 3-D Secure card payments', 'type' => Stancer\Auth::class], 'billingAddress' => ['desc' => 'The billing address links to the payment', 'onlyID' => \true, 'type' => Stancer\Address::class], 'capture' => ['desc' => 'Capture immediately the payment', 'type' => self::BOOLEAN], 'card' => ['changed' => [['sinceVersion' => Stancer\Enum\ApiVersion::VERSION_2, 'onlyID' => \true]], 'desc' => 'Card object', 'type' => Stancer\Card::class], 'country' => ['type' => self::STRING], 'currency' => ['changed' => [['sinceVersion' => Stancer\Enum\ApiVersion::VERSION_2, 'required' => \false]], 'desc' => 'Processed currency', 'required' => \true, 'type' => Stancer\Currency::class], 'customer' => ['changed' => [['sinceVersion' => Stancer\Enum\ApiVersion::VERSION_2, 'onlyID' => \true]], 'desc' => 'Customer object', 'type' => Stancer\Customer::class], 'dateBank' => ['desc' => 'Delivery date of the funds traded by the bank', 'restricted' => \true, 'type' => \DateTimeImmutable::class], 'description' => ['desc' => 'Payment description', 'size' => ['min' => 3, 'max' => 64], 'type' => self::STRING], 'device' => ['desc' => 'Customer\'s device object', 'type' => Stancer\Device::class], 'method' => ['desc' => 'Payment method used', 'restricted' => \true, 'type' => self::STRING], 'methodsAllowed' => ['desc' => 'List of payment methods allowed for this payment', 'list' => \true, 'type' => Stancer\Payment\MethodsAllowed::class], 'orderId' => ['desc' => 'Order identifier', 'size' => ['min' => 1, 'max' => 36], 'type' => self::STRING], 'refunds' => ['desc' => 'Array of refund objects', 'exportable' => \false, 'list' => \true, 'type' => Stancer\Refund::class], 'response' => ['desc' => 'Response of the bank processing', 'restricted' => \true, 'size' => ['min' => 2, 'max' => 4], 'type' => self::STRING], 'responseAuthor' => ['restricted' => \true, 'size' => ['fixed' => 6], 'type' => self::STRING], 'returnUrl' => ['desc' => 'URL to redirect back your customer after processing the payment', 'size' => ['min' => 1, 'max' => 2048], 'type' => self::STRING], 'sepa' => ['desc' => 'SEPA object', 'type' => Stancer\Sepa::class], 'shippingAddress' => ['desc' => 'The shipping address links to the payment', 'onlyID' => \true, 'type' => Stancer\Address::class], 'status' => ['desc' => 'Status of the payment', 'type' => Stancer\Payment\Status::class], 'uniqueId' => ['desc' => 'Unicity key', 'size' => ['min' => 1, 'max' => 36], 'type' => self::STRING]];
    /**
    ...
    public function refund(?int $amount = null) : static
    {
        ...
        if ($amount) {
            ...
            $params = [$amount / 100, $currency ? \strtoupper($currency->value) : '', ($this->getAmount() ?? 0) / 100, $this->getRefundedAmount() / 100];
            $message = '';
            if ($amount < 50) {
    ...
    public function setMethodsAllowed(array $methods) : static
    {
        ...
        if (!$method && $currency && \in_array(Stancer\Payment\MethodsAllowed::SEPA, $new, \true) && $currency !== Stancer\Currency::EUR) {
