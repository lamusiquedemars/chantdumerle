...
use Stancer\Scoped\Isolated\Stancer\Core\SearchObject;
/**
...
trait SearchTrait
{
    ...
    protected function search(string $class, string $property, array $terms, ?string $innerSearch = null) : \Generator
    {
        ...
        if ($innerSearch !== null) {
            if ($this->id === null) {
                throw new Stancer\Exceptions\InvalidSearchFilterException('You cannot search linked item before sending the object.');
            }
            $object = new SearchObject($this->id, $innerSearch, $this::ENDPOINT);
        }
        $until = null;
        $allowed = ['created', 'created_until', 'limit', 'start'];
        $allowed = \array_flip($allowed);
        [$params, $until] = $this->filterParams($terms, $allowed);
     ...
     */
    protected static function validateDateRelativeFilter(mixed $value, string $name, string $exception, bool $allowPeriod = \false) : int
    {
        $timestamp = $value;
